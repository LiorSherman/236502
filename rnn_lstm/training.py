...
import torch
from rnn_lstm.settings import DEFAULT_SEQ_LEN, DEFAULT_TRAINED_MODELS_DIR
...
class Trainer:
    ...
    def __init__(self, model, optimizer, criterion,
                 check_point_every=None, save=True, seq_len=DEFAULT_SEQ_LEN, device='cpu',
                 output=DEFAULT_TRAINED_MODELS_DIR):
    ...
    def fit(self, train_loader, validation_loader, epoches=50):
        ...
        with tqdm(total=epoches) as progress_bar:
            for epoch in range(epoches):
                progress_bar.set_description(desc=f"Epoch [{epoch + 1}/{epoches}]")
                ...
                for i, (sequences, labels) in enumerate(train_loader):
                    ...
                    progress_bar.set_description(
                        desc=f"Epoch [{epoch + 1}/{epoches}] Step [{i}/{len(train_loader)}]")
