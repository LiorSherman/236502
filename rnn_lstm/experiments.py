...
from rnn_lstm.settings import DEFAULT_DATASET, DEFAULT_SEQ_LEN, DEFAULT_HIDDEN_SIZE, DEFAULT_BATCH_SIZE, DEFAULT_LR, \
    DEFAULT_EXPERIMENTS_DIR


def kfold_hyperparam_experiment(exp_dir, k_folds=5, epoches=10, **hyperparams):
    # pre processing the dataset
    dataset = hyperparams.get('dataset') if hyperparams.get('dataset') else DEFAULT_DATASET
    seq_len = hyperparams.get('seq_len') if hyperparams.get('seq_len') else DEFAULT_SEQ_LEN
    ...
    with tqdm(total=k_folds) as progress_bar:
        ...
        for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
            ...
            trainloader = torch.utils.data.DataLoader(dataset, batch_size=DEFAULT_BATCH_SIZE, sampler=train_subsampler)
            testloader = torch.utils.data.DataLoader(dataset, batch_size=DEFAULT_BATCH_SIZE, sampler=test_subsampler)

            # Init the neural network
            device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
            hidden_size = hyperparams.get('hidden_size') if hyperparams.get('hidden_size') else DEFAULT_HIDDEN_SIZE
            model = LSTM(pre.outputs, hidden_size, pre.outputs, pre.mapping_file, device=device)
            model.to(device)

            # Initialize optimizer
            lr = hyperparams.get('lr') if hyperparams.get('lr') else DEFAULT_LR
...
def hidden_size_exp(kfolds=5, output=DEFAULT_EXPERIMENTS_DIR, show=True, **kwargs):
    dataset = kwargs.get('dataset') if kwargs.get('dataset') else DEFAULT_DATASET
    ...
    for size in hidden_sizes:
        ...
        result = kfold_hyperparam_experiment(k_folds=kfolds, epoches=5, hidden_size=size, dataset=dataset,
                                             exp_dir=size_dir)
...
def optimizers_exp(kfolds=5, output=DEFAULT_EXPERIMENTS_DIR, show=True, **kwargs):
    dataset = kwargs.get('dataset') if kwargs.get('dataset') else DEFAULT_DATASET
...
def seq_len_exp(kfolds=5, output=DEFAULT_EXPERIMENTS_DIR, show=True, **kwargs):
    dataset = kwargs.get('dataset') if kwargs.get('dataset') else DEFAULT_DATASET
    print(f"{dataset=}")
    ...
    for seq_len in seq_lens:
        ...
        result = kfold_hyperparam_experiment(k_folds=kfolds, epoches=5, seq_len=seq_len, dataset=dataset,
                                             exp_dir=seq_len_dir)
