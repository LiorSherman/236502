...
def normalize_parts(encoded_parts: List[List]):
    """
    Adding as much as rests needed to make all parts equally same with length
    :param encoded_parts: list of lists representing parts
    :return: all encoded parts padded with rests to be with equal length
    """
    max_duration = len(max(encoded_parts, key=len))
    for part in encoded_parts:
        to_add = max_duration - len(part)
...
class PreProcessor:
    ...
    """
    def __init__(self, dataset_path, output_dir, seq_len=DEFAULT_SEQ_LEN):
    ...
    def encode_songs(self):
        """
        encodes the song files into time-series-like music representation.
        """
        print("Encoding the dataset into time series representation...")
