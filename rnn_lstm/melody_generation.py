...
from rnn_lstm.dataset import MusicDataset
from rnn_lstm.settings import DEFAULT_SEQ_LEN, DEFAULT_HIDDEN_SIZE
...
class MelodyGenerator:
    def __init__(self, model_path, seq_len=DEFAULT_SEQ_LEN, device='cpu', hidden_size=DEFAULT_HIDDEN_SIZE):
        self.device = device
    ...
    def save_melody(self, melody: List[str], step_duration: float = 0.25, format: str = "midi",
        ...
        :param file_name: file path and name must end with .mid extension
        ...
        melody_dir = pathlib.Path(file_name).parent.resolve()
