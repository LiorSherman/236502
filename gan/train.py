...
class Trainer():
    ...
    def train(self, dataloader, epochs=2000, batch_size=64, repeat=5, display_step=10, device='cpu', **kwargs):
        """
        ...
        for epoch in tqdm(range(epochs)):
            ...
            for real in dataloader:
                ...
                batch_critic_fake_loss += fake_loss.item()
                batch_critic_real_loss += real_loss.item()
                batch_penalty_loss += 10 * penalty.item()
                batch_critic_loss += critic_w_loss.item()
                ...
                if epoch % repeat == 0:
                    ...
                    b_gloss.backward()
                    self.gen_opt.step()
                    exp_gen_loss += b_gloss.item() / (len(dataloader) / repeat)
